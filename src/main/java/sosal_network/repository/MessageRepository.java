...
public interface MessageRepository extends JpaRepository<ChatMessage, Long> {
    ...
    @Query(value = "select * from public.chat_message  where (user_from_id = ?1 and user_to_id = ?2) or (user_from_id = ?2 and user_to_id = ?1) order by time desc", nativeQuery = true)
