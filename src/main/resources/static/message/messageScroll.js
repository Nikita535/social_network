    ...
    xhr.onload = function (ev) {
        ...
        if (jsonResponse != null && jsonResponse[0].length > 0) {
            for (let i = 0; i < jsonResponse[0].length; i++) {
                ...
                let message = jsonResponse[1][i] != null ? jsonResponse[1][i]["userFrom"]["id"] === userFrom["id"] ? jsonResponse[1][i]["content"].length < 7 ? "Вы: " + jsonResponse[1][i]["content"] : "Вы: " + jsonResponse[1][i]["content"].substr(0, 7) + "..." : jsonResponse[1][i]["content"].length < 7 ? profile["name"] + ": " + jsonResponse[1][i]["content"] : profile["name"] + ": " + jsonResponse[1][i]["content"].substr(0, 7) + "..." : "";

                let source = friend["image"] != null ? '/image/' + friend["image"]["id"] : 'https://bootdey.com/img/Content/avatar/avatar6.png'


                friendMessage.innerHTML += "<div class=\"list-group lg-alt\">\n" + "                            <a class=\"list-group-item media friendListener\" style=\"text-decoration: \" id=\"" + friend["username"] + "\">\n" + "                                <div class=\"lv-avatar pull-left\">\n" + "                                    <img class=\"img-avatar mr-2\" src=\"" + source + "\" alt=\"\">\n" + "                                </div>\n" + "                                <div class=\"media-body\">\n" + "                                    <div class=\"list-group-item-heading\">" + profile["surname"] + " " + profile["name"] + "</div>\n" + "                                    <small class=\"list-group-item-text c-gray\">" + message + "</small>\n" + "                                </div>\n" + "                            </a>\n" + "                        </div>"
